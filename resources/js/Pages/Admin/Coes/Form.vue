...
const submit = () => {
    if (isEditing) {
        form.put(route('admin.coes.update', props.coe.id));
    } else {
                ...
                <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                        <form @submit.prevent="submit" class="space-y-6">
                            ...
                            <div>
                                <label for="full_content" class="block text-sm font-medium text-gray-700">Isi Lengkap (HTML/Teks)</label>
                                <textarea id="full_content" v-model="form.full_content" rows="10" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required></textarea>
